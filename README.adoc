...
image:https://ci.jenkins.io/buildStatus/icon?job=Infra%2Fpipeline-library%2Fmaster[link='https://ci.jenkins.io/job/Infra/job/pipeline-library/job/master/']
...
} else if (infra.isCiController()) {
    /* only perform untrusted action like a lint / build / test  */
}
----
