...
def call(userConfig = [:]) {
  def defaultConfig = [
    ...
    agentContainerImage: 'jenkinsciinfra/hashicorp-tools:0.5.1', // Version managed by updatecli
              ...
              stage('💸 Report estimated costs') {
                ...
                // try {
                  ...
                  if (commentComparison) {
                    ...
                    sh "export INFRACOST_DIFF=$(cat <<'END_HEREDOC'
Comparison between infracost plan & HCL methods: <details>\n\n```diff\n
${env.INFRACOST_COMPARISON}
\n```\n\n</details>
                    END_HEREDOC
                    )"
                    pullRequest.comment(env.INFRACOST_DIFF)
                    sh 'env | sort'
                  }
                // } catch(e) {
